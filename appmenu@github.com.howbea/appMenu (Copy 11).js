...
class MyPanelMenuButton extends PanelMenu.Button {
    _init() {
        ...
        this._bottomBar.connect('enter-event', () => {
        this._cancelHideDock();
        });

        this._bottomBar.connect('leave-event', () => {
        this._scheduleHideDock();
        });
    ...
    this._bottomBar = new St.BoxLayout({
        ...
        width: monitor.width,
        height: 320,
        style_class: 'dap-preview-dock',
        x: monitor.x,
        y: monitor.y + monitor.height, // ← 画面外
    });
    ...
    this._bottomBar.add_child(this._previewContainer);

    Main.layoutManager.uiGroup.add_child(this._bottomBar);
}
...
_createHotZone() {
    ...
    this._hotZone = new St.Widget({
        reactive: true,
        x: monitor.x,
        y: monitor.y + monitor.height - 1, //2, // ★ 最下端
        width: monitor.width,
        height: 1, //2,
        opacity: 0, // 完全透明
    });

    Main.layoutManager.uiGroup.add_child(this._hotZone);

    this._hotZone.connect('enter-event', () => {
        this._showDock();
    });

    this._hotZone.connect('leave-event', () => {
        this._scheduleHideDock();
    });
}
...
_showDock() {
    ...
    //this._reorderByWorkspaceWithSeparator();
...
_cancelHideDock() {
    ...
    _showWidget() {
        ...
        // animate in
        const finalY = monitor.y + monitor.height - 320; // 最終位置
...
const topBox = new St.BoxLayout({
    vertical: false,          // 横並び
    style: 'spacing:6px;',
    x_align: Clutter.ActorAlign.FILL,  // 左右いっぱいに広げる
    y_align: Clutter.ActorAlign.CENTER,
});
...
_reorderByWorkspace() {
    ...
    this._clones.sort((a, b) =>
        a.win.get_workspace().index() -
        b.win.get_workspace().index()
    );
    ...
    for (const c of this._clones) {
        const ws = c.win.get_workspace().index();

        if (lastWs !== -1 && ws !== lastWs) {
            const sep = new St.Widget({
                style_class: 'dap-ws-separator',
            });
            this._previewContainer.add_child(sep);
        }
...
_reorderByWorkspaceWithSeparator() {
    const wm = global.workspace_manager;
...
export class AppMenu extends PopupMenu.PopupMenu {
    ...
    constructor(sourceActor, side = St.Side.TOP, params = {}) {
        ...
        this._mypanelMenuButtonItem = this.addAction(_('Window Previews'), () => {
            ...
            this._mypanelMenuButton.toggleDock();
            this._mypanelMenuButton._cancelHideDock();
            this._mypanelMenuButton._scheduleHideDock2();
        });
