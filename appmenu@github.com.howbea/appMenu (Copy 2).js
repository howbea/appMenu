...
class MyPanelMenuButton extends PanelMenu.Button {
    ...
    //
    // 🔵 クリック：overlay クリックは閉じる（AltTab と同じ）
    //
    ...
    //
    // 🔵 Super / ホットコーナー / Overview トリガーを無効化
    //     AltTab と同じく、「何も起こらない」扱いにする
    //
    if (type === Clutter.EventType.KEY_PRESS) {

        // Super キー（左右）を完全無効化
        if (symbol === Clutter.KEY_Super_L ||
            symbol === Clutter.KEY_Super_R) {

            // 押しても何もしない（GNOME 公式 AltTab と同じ挙動）
            event.stop();
            return Clutter.EVENT_STOP;
        }
        ...
        //
        // pushModal と同じ処理：
        // → アプリにフォーカスが戻らないよう permanently grab
        //
        global.display.set_key_focus(null);
        try { this.bottomBar.grab_key_focus(); } catch (e) {}
