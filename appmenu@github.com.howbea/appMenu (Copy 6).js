...
export const MyPanelMenuButton = GObject.registerClass(
class MyPanelMenuButton extends PanelMenu.Button {
    ...
    _showWidget() {
        ...
        this._bottomBar = new St.BoxLayout({
            ...
            vertical: false,
            x: monitor.x,
            y: monitor.y + monitor.height - 320,
            width: monitor.width,
            height: 320,
        ...
        this._reorderByReverseAltTab();
        //this._reorderByReverseMRU();
        //this._reorderByReverseMRUGroupedByApp();
        ...
        // animation
        const finalY = monitor.y + monitor.height - 320;
        const startY = finalY + 160;
    ...
    _closeWidget() {
        ...
        if (!this._bottomBar || !this._overlay) return;

        const overlay = this._overlay;
        const bottomBar = this._bottomBar;
        const endY = bottomBar.y + 80;
        ...
        bottomBar.ease({
            ...
            onComplete: () => {
                [this._previewContainer, this._previewWrapper, this._bottomBar, this._overlay].forEach(w => {
                    if (w && w.get_parent())
                ...
                if (this._overviewSignalId) {
                    try { Main.overview.disconnect(this._overviewSignalId); } catch(e) {}
                    this._overviewSignalId = 0;
                }
            },
        });

        try { overlay.ease({ opacity: 0, duration: 200, mode: Clutter.AnimationMode.EASE_IN_QUAD }); } catch(e) {}
    }
    ...
    _populateClones() {
        ...
        for (let i = 0; i < wins.length; i++) {
            ...
            const icon = app ? app.create_icon_texture(52) : new St.Icon({
...
if (title.clutter_text)
    title.clutter_text.set_ellipsize(Pango.EllipsizeMode.END);
            ...
            icon.set({ x_expand: false, y_expand: false, x_align: Clutter.ActorAlign.START, y_align: Clutter.ActorAlign.CENTER });
            topBox.add_child(icon);
            topBox.add_child(title);
            ...
            itemBox.add_child(topBox);
            itemBox.add_child(cloneContainer);

            itemBox.connect('enter-event', () => itemBox.set_style('spacing:6px; padding:6px; border-radius:10px; background-color: rgba(255,255,255,0.35);'));
            itemBox.connect('leave-event', () => itemBox.set_style('spacing:6px; padding:6px; border-radius:10px;'));
    ...
    _reorderByReverseMRUGroupedByApp() {
    const tracker = Shell.WindowTracker.get_default();

    // Alt+Tab(MRU) → 逆順
    const reverseMRU = global.display.get_tab_list(
    ...
    for (const c of this._clones)
        cloneMap.set(c.win, c);

    const usedApps = new Set();
    const newClones = [];

    for (const win of reverseMRU) {
        const clone = cloneMap.get(win);
        if (!clone)
            continue;
        ...
        // 最初に出てきた appId が「逆MRU順」
        if (!usedApps.has(appId)) {
            usedApps.add(appId);

            // 同じアプリのウィンドウを reverseMRU 順で全部追加
            for (const w of reverseMRU) {
                const c = cloneMap.get(w);
                if (!c)
                    continue;

                const a = tracker.get_window_app(w);
                const id = a?.get_id() ?? w.get_wm_class();
    ...
    this._previewContainer.remove_all_children();
    for (const c of this._clones)
        this._previewContainer.add_child(c.box);
}

_reorderByReverseAltTab() {
    ...
    for (let i = mru.length - 1; i >= 0; i--) {
        const c = cloneMap.get(mru[i]);
        if (c)
            reversed.push(c);
    }

    this._clones = reversed;
...
_focusActiveWindow() {
    ...
    if (idx >= 0) {
        ...
        this._updateHighlight();
        try {
            this._clones[idx].box.grab_key_focus();
        } catch (e) {}
    }
}
...
const mypanelMenuButton = new MyPanelMenuButton();
...
export class AppMenu extends PopupMenu.PopupMenu {
    ...
    constructor(sourceActor, side = St.Side.TOP, params = {}) {
        ...
        this._mypanelMenuButtonItem = this.addAction(_('Show Previews'), () => {
    ...
    setApp(app) {
        ...
        this.addAction(_('Show Previews'), () => {
            mypanelMenuButton._showWidget();
        });

        this.addMenuItem(new PopupMenu.PopupSeparatorMenuItem());

        this.addAction(_('Change Background…'), () => {
            Shell.AppSystem.get_default()
                .lookup_app('gnome-background-panel.desktop')
                ?.activate();
            Main.overview.hide();
        });

        this.addAction(_('Display Settings'), () => {
            Shell.AppSystem.get_default()
                .lookup_app('gnome-display-panel.desktop')
                ?.activate();
            Main.overview.hide();
        });
