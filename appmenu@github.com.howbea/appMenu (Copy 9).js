...
_populateClones() {
    ...
    for (let wsIndex = 0; wsIndex < n; wsIndex++) {
        const ws = wm.get_workspace_by_index(wsIndex);
        const wins = ws.list_windows()
            .filter(w => !w.skip_taskbar && tracker.get_window_app(w));
        ...
        /* ★ WS 境界の Dock セパレーター */
        if (!first) {
            this._previewContainer.add_child(
                createDockSeparator(MAX_H + 40)
            );
        }
        first = false;
        ...
        for (const win of wins) {
            ...
            /* ↓↓↓ 以下は今の itemBox 作成コードをそのまま ↓↓↓ */

            const app = tracker.get_window_app(win);
            const icon = app.create_icon_texture(52);
            ...
            // topBox / title / cloneContainer / itemBox
            // hover / click
            // （ここは一切変更不要）
