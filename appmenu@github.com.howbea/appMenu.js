...
class MyPanelMenuButton extends PanelMenu.Button {
    _init() {
        ...
        this._hotZone = null;
        this._shownByMouse = false;
        this._hoverTimeoutId = 0;
        this._createHotZone();
        this._altTabMode = false;
    ...
    this._hotZone = new St.Widget({
        reactive: true,
        x: monitor.x,
        y: monitor.y + monitor.height - 1, // 最下端 1px
        width: monitor.width,
        height: 1,
        opacity: 0, // 完全透明
    });
    ...
    _showWidget() {
        ...
        this._bottomBar = new St.BoxLayout({
            ...
            width: monitor.width,
            height: 320,
            style: 'background-color: rgba(222, 221, 218, 1); padding: 12px;',
        });
        
        this._bottomBar.connect('enter-event', () => {
    // Dock 上に戻ってきたら hide をキャンセル
...
topBox.add_child(title);
    ...
    _onCapturedEvent(actor, event) {
        ...
        if (event.type() === Clutter.EventType.KEY_RELEASE && this._altTabMode) {
    ...
    startAltTab(backward) {
    if (Main.overview.visible)
        return;

    this._altTabMode = true;

    this._showWidget();   // ← 既存ドックをそのまま使う
